...
public class filterDraftButtonSanity {
	...
	List<String> editorcorButtons = new ArrayList<String>(Arrays.asList("Editar", "Borrar", "Destacar"));// Unbrand Coordinator
	...
	public void draft() throws Exception {
		...
		status2.selectByVisibleText("Borrador");
	...
	public void buttonStatus(String role) throws Exception {
		...
		System.out.println("postID is-->>" + postID + "   postType is-->>" + postTypeStatus	+ "   postTitle is-->>" +postTitle + "    <<-----No action buttons for club post---->>");
				...
				if (role.equalsIgnoreCase("Branded Collaborator") && (postTypeStatus.equalsIgnoreCase("Club"))&& (driver.findElement(By.xpath(".//*[@id='row-" + postID + "']/td[3]/a")).getText().equalsIgnoreCase(roleType.Authorname))) {
					List<WebElement> BColButtons = driver.findElements(By.xpath(".//*[@id='row-" + postID + "']" +prop.getProperty("buttonXpath")));
